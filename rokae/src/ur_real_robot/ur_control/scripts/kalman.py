...
class Kalman:
    ...
    def __init__(self, itr_num_):
        ...
        # self.P0=np.diag([0.05,0.05,0.05,0.05,0.05,0.05])
    ...
    def iteration(self,pose):
        ...
        # e=np.dot(r.as_matrix(),np.array([0,0,1]).reshape([3,1]))
        # xx=e[0,0]
        # yy=e[1,0]
        # zz=e[2,0]
        # while (zz>-0.996):
        ...
        #     yy=yy/2
        #     zz=zz-1
        #     sqrt=math.sqrt(pow(xx,2)+pow(yy,2)+pow(zz,2))
        #     xx=xx/sqrt
        #     yy=yy/sqrt
        #     zz=zz/sqrt           
        ...
        if (s==0):
            ...
            if(self.itr_time==self.itr_num):
                ...
                # print('ang_dist:')
                # print(ang_dist)
                # if (dist<0.001) and (ang_dist>0.999):
    ...
    def plot(self):
        if (self.itr_time!=0):
            ...
            # np.savetxt("filt_25.csv",self.X1_np,delimiter=",")
            # np.savetxt("obs_25.csv",self.Z1_np,delimiter=",")
